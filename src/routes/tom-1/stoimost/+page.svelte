...
<article>
	...
	<p class="mt-3 max-w-prose text-ink-soft">
		Модель показывает продукт одного рабочего дня — рабочего или целого коллектива, если считать его
		как одного совокупного работника. За один час живого труда рабочий создаёт новую стоимость 100
		₽. Рабочий день длится 12 часов, поэтому живой труд за день всегда создаёт ровно
		<strong>1200 ₽</strong> новой стоимости (12 × 100). Эта сумма распадается на
	...
	<p class="mt-3 max-w-prose text-ink-soft">
		<strong class="text-const">Постоянный капитал c</strong> — стоимость средств производства
		(сырья, машин, зданий), перенесённая на продукт без изменения величины: отсюда и название
		«постоянный». Живой труд этого дня её не создаёт, поэтому <code>c</code> меняется независимо от v
		и m, в диапазоне от 0 до 2000 ₽.
	</p>
	<p class="mt-3 max-w-prose text-ink-soft">
		При увеличении переменного капитала <code>v</code> общая вновь созданная стоимость не растёт:
		она всегда равна 1200 ₽, меняется только её деление между необходимым и прибавочным трудом. Рост
		<code>v</code> здесь — это не наём новых рабочих, а рост зарплаты тех же самых: их число и длина дня
		не меняются.
	</p>
	...
	<div class="mt-10 space-y-6">
		...
		<div>
			...
			<p class="mt-1 text-xs text-ink-soft">
				...
				той же суммы: v + m = {formatNumber(DAILY_LIVING_LABOUR_VALUE)} всегда, и обе величины остаются
				больше нуля.
			</p>
		</div>
	</div>
		...
		<table class="w-full min-w-[360px] border-collapse text-left text-sm">
			...
			<tbody>
				<tr class="border-b border-ink/10">
					<td class="py-2 pr-3"><code class="font-semibold">W</code> — стоимость продукта дня</td>
